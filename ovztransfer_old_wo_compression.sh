...
function migrate() {
    ...
    [ $? -eq 0 ] && xattrs="--xattrs"
    vzctl --quiet exec $veid tar --numeric-owner $xattrs -c -C $tmpdir ./ 2>/dev/null | ssh $ssh_opts root@$target tar --numeric-owner $xattrs -x -C /vz/root/$target_veid > /dev/null 2>&1
